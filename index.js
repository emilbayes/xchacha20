var wasm = require('./load-wasm')(require('fs').readFileSync('./hchacha20.wasm'))
...
for (var i = 0; i < testvectors.length; i++) {
  var elm = testvectors[i]

  var outputBuf = Buffer.from(elm[2], 'hex')
  var keyBuf = Buffer.from(elm[0], 'hex')
  var inputBuf = Buffer.from(elm[1], 'hex')

  // 32 | 16 | 32 | 0
  wasm.memory.fill(0, 0, OUTPUTBYTES + INPUTBYTES + KEYBYTES + CONSTBYTES)
  wasm.memory.set(inputBuf, OUTPUTBYTES)
  wasm.memory.set(keyBuf, OUTPUTBYTES + INPUTBYTES)

  wasm.exports.hchacha20(0, OUTPUTBYTES, OUTPUTBYTES + INPUTBYTES, NULL)

  console.log(outputBuf.equals(wasm.memory.slice(0, OUTPUTBYTES)))
}
